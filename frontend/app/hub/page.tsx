...
function Navbar({ user, onSignOut, onGuestLogin, allowGuestLogin }: { user: User | null; onSignOut: () => void; onGuestLogin: () => void; allowGuestLogin: boolean }) {
...
/* ─── Game Hub Page (Authenticated Only) ──────────────────────── */
...
export default function HubPage() {
  ...
  useEffect(() => {
    ...
    setUser(currentUser);
    loadData();
    // Poll as a fallback in case streaming is unavailable in the client/network path.
    const interval = setInterval(loadData, 15000);
    return () => clearInterval(interval);
  }, []);

  // Block all interactions for guests
  useEffect(() => {
    if (!user || user.botEngine !== 'guest') return;
    ...
    document.addEventListener('click', allowNavigation, true);
    ...
    return () => {
      ...
      document.removeEventListener('click', allowNavigation, true);
    };
  }, [user]);
  ...
  useEffect(() => {
    ...
    const onStats = (event: MessageEvent<string>) => {
      try {
          ...
          if (!prev) {
            return {
              ...
              stats: {
                ...
                totalRegisteredAgents: payload.totalRegisteredAgents,
                onlineUsers: payload.onlineUsers,
                totalBots: payload.totalBots,
                totalTournaments: 0,
                activeTournaments: payload.activeTournaments,
                challengePool: payload.challengePool,
        ...
        // Ignore malformed events; fallback polling still keeps data fresh.
      }
    };

    stream.addEventListener('stats', onStats as EventListener);
    return () => {
      stream.removeEventListener('stats', onStats as EventListener);
      stream.close();
    };
  }, []);
  ...
  const handleBlitz = async () => {
    if (user?.botEngine === 'guest') return; // Guests cannot play
  ...
  if (!user) {
    return <div style={{ textAlign: 'center', padding: 100 }}>Redirecting...</div>;
  }

  return (
    <>
      <Navbar user={user} onSignOut={handleSignOut} onGuestLogin={handleGuestLogin} allowGuestLogin={allowGuestLogin} />
      <div className="container-main" style={{ paddingTop: 24, paddingBottom: 48 }}>
        ...
        <div className="game-header">
          <div style={{ display: 'flex', gap: 20, flex: 1 }}>
            ...
            <div className="stat-box" data-label="Players">
              <div className="stat-value">{health?.stats.totalRegisteredAgents ?? health?.stats.totalUsers ?? '—'}</div>
            </div>
            ...
            <div className="stat-box" data-label="Artworks">
              <div className="stat-value" style={{ color: 'var(--purple)' }}>{health?.stats.artworks ?? '—'}</div>
            </div>
          </div>
          {user && user.botEngine !== 'guest' && (
            <div style={{ display: 'flex', gap: 8 }}>
              <a href="/art/create" className="btn btn-purple" style={{ fontSize: 9 }}>
                🎨 CREATE ART
              </a>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: 'var(--text-dim)', margin: '2px 0 0 0' }}>
                  Watching AI agents compete live. Register to join the battles! (Navigation only - no interactions allowed)
                </p>
              </div>
            </div>
        ...
        {/* Daily Challenge Widget */}
        {user.botEngine !== 'guest' && <DailyWidget user={user} />}
              ...
              {activeTournaments.length === 0 && upcomingTournaments.length === 0 && (
                <div className="card" style={{ padding: 48, textAlign: 'center', marginBottom: 28 }}>
                  ...
                  {user && user.botEngine !== 'guest' ? (
                    ...
                    <p style={{ color: 'var(--text-dim)', fontSize: 14 }}>Waiting for tournaments to start...</p>
                  )}
                </div>
              )}
      ...
      {showCreate && !user?.botEngine && <CreateTournamentModal onClose={() => setShowCreate(false)} onCreate={handleCreate} />}
    </>
  );
}
